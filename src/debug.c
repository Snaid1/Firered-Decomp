//CREDITS
//Snaid1:               https://github.com/Snaid1/Firered-Decomp/tree/DebugMenu
//CODE USED FROM:
...
//#include "constants/battle_frontier.h" //uncomment when battle frontier implemented
...
static void Debug_DestroyMenu_Full(u8 taskId)
{
    ...
    {
        ClearStdWindowAndFrameToTransparent(gTasks[taskId].data[2], FALSE);
...
static void DebugAction_DestroyExtraWindow(u8 taskId)
{
    ...
    ClearStdWindowAndFrameToTransparent(gTasks[taskId].data[2], TRUE);
...
// uncomment and adjust if a clock is placed in game
//static void DebugAction_Util_CheckWallClock(u8 taskId)
//{
//    Debug_DestroyMenu_Full(taskId);
//    LockPlayerFieldControls();
...
//commented out and left due to future plans to implement battle frontier in firered
/*static void DebugAction_FlagsVars_ToggleFrontierPass(u8 taskId)
{
    // Sound effect
...
static void DebugAction_Fill_PCItemStorage(u8 taskId)
{
    ...
    {
        if (!CheckPCHasItem(itemId, MAX_ITEM_CAPACITY))
            AddPCItem(itemId, MAX_ITEM_CAPACITY);
    }
}
...
static void DebugAction_Fill_PocketItems(u8 taskId)
{
    ...
    for (itemId = 1; itemId < ITEMS_COUNT; itemId++)
    {
        if (ItemId_GetPocket(itemId) == POCKET_ITEMS && CheckBagHasSpace(itemId, MAX_ITEM_CAPACITY))
            AddBagItem(itemId, MAX_ITEM_CAPACITY);
    }
}
...
static void DebugAction_Fill_PocketPokeBalls(u8 taskId)
{
    ...
    {
        if (CheckBagHasSpace(itemId, MAX_ITEM_CAPACITY))
            AddBagItem(itemId, MAX_ITEM_CAPACITY);
    }
}
static void DebugAction_Fill_PocketTMHM(u8 taskId)
